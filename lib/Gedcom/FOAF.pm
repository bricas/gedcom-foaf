...
sub as_foaf {
	...
	for my $photo ( $self->tag_value( 'PHOT' ) ) {
		...
		$depic->setAttribute( 'rdf:resource' => $baseurl . 'photos/' . $photo );
...
sub as_foaf {
	...
	$label->appendText( join( ' ', 'The Family of', $husband, 'and', $wife ) );
...
sub _foaf_seealso {
	...
	$group->setAttribute( 'rdf:about' => $baseurl . $self->xref . '.xml#' . $self->xref );

	my $seealso = XML::LibXML::Element->new( 'rdfs:seeAlso' );
	$seealso->setAttribute( 'rdf:resource' => $baseurl . $self->xref . '.xml' );

	my $member = XML::LibXML::Element->new( 'foaf:member' );
	$member->setAttribute( 'rdf:resource' => $baseurl . $person->xref . '.xml#' . $person->xref );
